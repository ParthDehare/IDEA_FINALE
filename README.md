...
**Scoring Weights:**
- BehaviourWatch: 45% | RegulatoryAI: 45% | VendorGuard: 5% | NetworkIntel: 2% | FundFlow: 2% | ComplaintSignal: 1%

**Decision Engine:**
- CBSI ≥ 80 → **ISOLATE** (auto-generate evidence package, trigger SMS/Email)
- CBSI ≥ 50 → **MONITOR**
- CBSI < 50 → **PASS**
...
│   ├── generate_demo_data.py        # Perfect-storm synthetic fraud injection
│   └── ...
```
...
All data is **100% synthetic**, generated by our team using `scripts/data_generator_v2.py` and `generate_demo_data.py`. No real banking or customer data was used.
...
- **Anomalous patterns** injected for ~5% of employees (suspicious login hours, unusual transaction volumes, honeypot access attempts, VIP target accounts, Midnight harvest bulk downloads, Bribe keywords).
...
- 📱 **Multi-Channel Alerting (NEW)** — Instant Twilio SMS and SMTP Email dispatch when critical fraud (CBSI > 80) is detected.
- 🌪️ **"Perfect Storm" Fraud Generation (NEW)** — Advanced simulation logic that guarantees rule-based agent triggers (Bribe keywords, Tor IP addresses, Midnight exports) for flawless judge-ready demonstrations.
