...
def inject_maker_checker(employees, ip_pool, rng, n_instances=20):
    dates   = spread_dates(n_instances, start=HISTORICAL_CUTOFF)
    rows    = []
    for ts in dates:
...
def inject_zero_day_loan(employees, ip_pool, rng, n_bursts=25, loans_per_burst=15):
    ...
    """
    dates    = spread_dates(n_bursts, start=HISTORICAL_CUTOFF)
    rows     = []
    for i, ts in enumerate(dates):
        clerk   = employees[employees["emp_class"] == "CLERK"].sample(1).iloc[0]
        manager = employees[employees["emp_class"] == "MANAGER"].sample(1).iloc[0]
...
def inject_atm_harvest(employees, ip_pool, rng, n_events=35, withdrawals_per_event=8):
    ...
    """
    dates   = spread_dates(n_events, start=HISTORICAL_CUTOFF)
    rows    = []
    for i, ts in enumerate(dates):
        clerk      = employees[employees["emp_class"] == "CLERK"].sample(1).iloc[0]
...
def validate(df: pd.DataFrame) -> None:
    ...
    assert 0.01 <= fraud_rate <= 0.25,               f"Fraud rate {fraud_rate:.4f} out of range"
